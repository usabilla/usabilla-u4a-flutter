...
class FlutterUsabillaPlugin : FlutterPlugin, MethodCallHandler, ActivityAware, EventChannel.StreamHandler {
            ...
            Usabilla.sharedFlowClosingForm.collectLatest {
                if (it.formType == com.usabilla.sdk.ubform.sdk.form.FormType.PASSIVE_FEEDBACK) {
                    ...
                    (activity as? FragmentActivity)?.let { fragmentActivity ->
                        val supportFragmentManager: FragmentManager = fragmentActivity.supportFragmentManager
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            ...
            "prePopulateEmailComponent" -> prePopulateEmailComponent(call, result)
            "setFooterLogoClickable" -> setFooterLogoClickable(call, result)
            "getPlatformVersion" -> result.success(android.os.Build.VERSION.RELEASE)
            else -> result.notImplemented()
        }
    }

    private fun attachReceivers() {
        collectClosingFormSharedFlow()
    }

    private fun detachReceivers() {
    ...
    private fun takeScreenshot(view: View): Bitmap? {
        ...
        if (registry is FlutterView) {
            bitmap = (registry as FlutterView).bitmap
        } else if (registry is FlutterRenderer) {
    ...
    private fun getDefaultDataMasks(result: Result) {
        result.success(com.usabilla.sdk.ubform.UbConstants.DEFAULT_DATA_MASKS)
    }
    ...
    private fun prePopulateEmailComponent(call: MethodCall, result: Result) {
        val email = getArgumentFromCall<String>(call, "email")
        val editable = getArgumentFromCall<Boolean>(call, "editable")
        Usabilla.prePopulateEmailComponent(email, editable)
        result.success(null)
    }

    private fun setFooterLogoClickable(call: MethodCall, result: Result) {
        val clickable = getArgumentFromCall<Boolean>(call, "clickable")
